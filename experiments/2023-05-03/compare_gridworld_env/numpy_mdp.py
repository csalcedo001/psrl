...
from psrl.envs.env import Env
from psrl.envs.utils import get_grid_from_file


class GridworldEnvNumpy(Env):
    def __init__(self, config):
        ...
        self.p = self.r = None
        self.p, self.r = self.get_p_and_r()
    
    def reset(self):
        self.pos = self.start_states[np.random.randint(0, len(self.start_states))]
        self.state = self._get_state_from_pos(self.pos)
    ...
    def get_p_and_r(self):
        if self.p is not None and self.r is not None:
